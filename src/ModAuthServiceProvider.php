    ...
    public function boot()
    {
        ...
        Validator::extend('current_secure_pid', function ($field, $value, $parameters) {
            if (Auth::check()) {
                ...
                if (! empty($secure->personal_id) && $secure->personal_id != $value) {
                    return false;
                }
                return true;
            }
        ...
        Validator::extend('current_secure_phone', function ($field, $value, $parameters) {
            if (Auth::check()) {
                $uid = Auth::user()->id;
                $secure = UserSecure::find($uid);
                if (! empty($secure->phone) && ! empty($secure->phone_verified) && $secure->phone_verified && $secure->phone_verified != $value) {
                    return false;
                }
                return true;
            }
        ...
        Validator::extend('current_secure_email', function ($field, $value, $parameters) {
            if (Auth::check()) {
                $uid = Auth::user()->id;
                $secure = UserSecure::find($uid);
                if (! empty($secure->email) && ! empty($secure->email_verified) && $secure->email != $value) {
                    return false;
                }
                return true;
            }
        ...
        Validator::extend('current_secure_pass2', function ($field, $value, $parameters) {
            if (Auth::check()) {
                $uid = Auth::user()->id;
                $secure = UserSecure::find($uid);
                if (! empty($secure->pass2) && ! Hash::check($value, $secure->pass2)) {
                    return false;
                }
                return true;
            }
        ...
        Validator::extend('current_secure_question', function ($field, $value, $parameters) {
            if (Auth::check()) {
                $uid = Auth::user()->id;
                $secure = UserSecure::find($uid);
                if (! empty($secure->question) && $secure->question != $value) {
                    return false;
                }
                return true;
            }
        ...
        Validator::extend('current_secure_answer', function ($field, $value, $parameters) {
            if (Auth::check()) {
                $uid = Auth::user()->id;
                $secure = UserSecure::find($uid);
                if (! empty($secure->answer) && ! Hash::check($value, $secure->answer)) {
                    return false;
                }
                return true;
            }
